...
export const addMemberByHost = async (
  req: AuthRequest,
  res: Response
) => {
  try {
...
export const getAllPublicGroups = async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user.id;
...
export const getMyGroupRequests = async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user.id;
    const groups = await Chat.find({
      groupAdmin: userId,
...
export const updateGroupImage = async (req: AuthRequest, res: Response) => {
  try {
    const { groupId } = req.params;
    const adminId = req.user.id;
